...
- It needs a kubeconfig with rights to create and delete the resources of that release in the namespaces involved. Those are a Job, a Deployment and a Service for the SSH side, Secrets, ServiceAccounts and, when enabled, NetworkPolicies. The RBAC example in [Bucket backup and restore](backup-restore.md#scheduled-backups) lists them.
...
- Network policies for the three components are off by default and opt-in per component through chart values, because most clusters do not need them. In a default-deny cluster without them, the SSH connection never establishes and every network strategy fails.
