...
func buildRemotePath(req *Request) (string, error) {
	...
	if !safeBucketSegment.MatchString(req.Name) {
		return "", fmt.Errorf(
			"--name %q contains invalid characters (allowed: alphanumeric, hyphens, underscores, dots)", req.Name)
	}

	if req.Prefix != "" && !safeBucketSegment.MatchString(req.Prefix) {
		return "", fmt.Errorf(
			"--prefix %q contains invalid characters (allowed: alphanumeric, hyphens, underscores, dots)", req.Prefix)
	}
...
func handleJobCompletion(
	...
	if err := k8s.WaitForJobCompletion(ctx, kubeClient, namespace, jobName,
		false /* rclone progress bar not yet supported */, req.Writer, logger); err != nil {
