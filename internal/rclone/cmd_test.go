...
func TestBuildCommand_Backup(t *testing.T) {
	...
	assert.Equal(t, "rclone sync /data remote:my-bucket/default/my-pvc/", result)
}
...
func TestBuildCommand_Restore(t *testing.T) {
	...
	result, err := cmd.Build()
	require.NoError(t, err)
	assert.Equal(t, "rclone sync remote:my-bucket/default/my-pvc/ /data", result)
}
...
func TestBuildCommand_WithConfigPath(t *testing.T) {
	...
	result, err := cmd.Build()
	require.NoError(t, err)
	assert.Equal(t, "rclone sync --config /etc/rclone/rclone.conf /data remote:bucket/", result)
}
...
func TestBuildCommand_WithExtraArgs(t *testing.T) {
	...
	result, err := cmd.Build()
	require.NoError(t, err)
	assert.Equal(t, "rclone sync /data remote:bucket/path/ --dry-run --verbose", result)
}
