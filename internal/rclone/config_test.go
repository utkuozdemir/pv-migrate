...
func TestGenerateConfig_S3_WithCredentials(t *testing.T) {
	...
	assert.Contains(t, conf, "no_check_bucket = true")
...
func TestGenerateConfig_S3_CustomProvider(t *testing.T) {
	t.Parallel()
	...
	conf, err := rclone.GenerateConfig(opts)
	require.NoError(t, err)
	assert.Contains(t, conf, "provider = AWS")
}
...
func TestGenerateConfig_GCS_BucketPolicyOnlyDisabled(t *testing.T) {
	t.Parallel()

	bucketPolicyOnly := false
	opts := rclone.ConfigOptions{
		Backend:             rclone.BackendGCS,
		GCSBucketPolicyOnly: &bucketPolicyOnly,
	}

	conf, err := rclone.GenerateConfig(opts)
	require.NoError(t, err)
	assert.NotContains(t, conf, "bucket_policy_only")
}
